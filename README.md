...
　　注)互換モニタでは、スピーカーから正常に音が出ません。正常に音を出したい場合は、rom.hをTK-80正規モニタのダンプリストに差し替えてコンパイルし直してください。
...
2022.5.6 コメント再追加
         ...
         スピーカー出力の動作条件を付記。rom.hをTK-80正規モニタのダンプリストに差し替えてコンパイルが必要。
